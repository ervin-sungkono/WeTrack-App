...
export default function SidebarLink({ label, url, icon, baseUrl, open }){
    ...
    return(
        <Link href={pathString} className="group">
            <div className={`flex items-center gap-2 px-4 py-2.5 ${pathname === pathString ? "bg-white text-basic-blue" : "group-hover:bg-dark-blue/10 text-dark-blue group-hover:text-basic-blue"} rounded-md transition-colors duration-300`}>
                <div>{icon}</div>
