...
export default function SidebarLink({ label, url, icon, baseUrl, open }){
    ...
    return(
        <Link href={pathString} className="group select-none">
            <div className={`flex items-center gap-2 px-4 py-2.5 ${pathname === pathString ? "bg-basic-blue/10 text-basic-blue cursor-default" : "group-hover:bg-dark-blue/10 text-dark-blue group-hover:text-basic-blue"} rounded-md transition-colors duration-300`}>
                <div>{icon}</div>
