    ...
    return (
        <>
            ...
            <div className="flex flex-row gap-2 md:gap-4">
                {list.map((item, index) => (
                    <TeamItem key={index} selectUpdate={selectUpdate} setSelectUpdate={setSelectUpdate} selectDelete={selectDelete} setSelectDelete={setSelectDelete} editMode={edit} status={listType==="pending" ? "pending" : "accepted"} {...item} />
                ))}
            </div>
        </>
    )
